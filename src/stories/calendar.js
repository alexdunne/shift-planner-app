...
storiesOf("Calendar", module)
  .add(
    ...
    withState({ month: now.getMonth() + 1, year: now.getFullYear() })(({ store }) => (
      <Container>
        <Row>
          <Col>
            <input
              ...
              onChange={event => store.set({ month: parseInt(event.target.value, 10) })}
            />
          </Col>
          <Col>
            <input
              ...
              onChange={event => store.set({ year: parseInt(event.target.value, 10) })}
            />
          </Col>
        </Row>
        <Row>
